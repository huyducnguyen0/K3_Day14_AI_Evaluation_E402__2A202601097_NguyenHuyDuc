...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the response from 0.0 to 1.0 for each rubric criterion. "
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for result in scores_batch:
            ...
            if isinstance(first, (int, float)) and isinstance(second, (int, float)):
                positional_comparisons.append(float(first) > float(second))
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            suggestion = (
                ...
                else "Review and improve the affected pipeline stage"
            )
            values = (
                f"F{index + 1:03d}",
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": "Implement a grounding check to reject claims unsupported by retrieved context",
            "irrelevant": "Add intent-focused prompt examples so answers directly address the question",
            "incomplete": "Improve retrieval coverage and prompt the generator to include all required facts",
            "off_topic": "Strengthen intent classification and add an off-topic response guardrail",
            "refusal": "Calibrate safety guardrails to reduce unnecessary refusals on answerable requests",
            "unknown": "Review low-scoring examples and label their failure modes for targeted fixes",
        }
        ...
        general_suggestions = [
            "Add failing examples to the golden dataset and rerun the benchmark after each change",
            "Track per-metric trends in CI and block releases when regression thresholds are exceeded",
            "Review retrieval and generation traces for the lowest-scoring benchmark cases",
        ]
